...
The window rolls from the last thunderclap, not from a wall clock, so the indulgences space themselves against the music rather than the minute hand. The behavior is fully deterministic: the same playing gets the same thunderclaps, and the `seed` keeps the effect reproducible alongside the rest of the stochastic palette; see [Seeds](/miditool/configuration/seeds/).
