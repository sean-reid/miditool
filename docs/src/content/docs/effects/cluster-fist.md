...
Henry Cowell got these sounds with fists and forearms in *The Tides of Manaunaun*, notating bands of adjacent keys as single gestures; here the band comes from one key. Each note-on becomes `width` member keys drawn from the kind's set: every key (`"chromatic"`), the piano's white keys, its black keys, or the members of a [sieve](/miditool/effects/sieve/) (`kind="sieve" sieve="..."`). The `anchor` places your key at the bottom, center, or top of the fist, and the cluster fades away from it: the played key keeps your velocity, and each other member is scaled down by `rolloff` per step of distance. The played key is always included even when the set does not contain it (a black-key cluster anchored on a white key keeps its white anchor), and a side that runs off the keyboard truncates, shrinking the cluster.
