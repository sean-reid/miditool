...
Like the other generators, `continuum` runs on its own clock rather than waiting for input, seeded so the same seed and the same playing replay the same stream (the seed only matters for `order="random"`; the other orders are already deterministic). If ticks run late, the machine catches up at most 4 notes and then skips the rest, so time never bunches. It stops cleanly on a scene switch and on shutdown; every note it starts carries its own note-off.
